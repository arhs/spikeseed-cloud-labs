...
SECURITY_GROUP_TAG_FOR_GLOBAL_HTTP = { 'SecurityGroupType': 'cloudfront_g', 'AutoUpdate': 'true', 'Protocol': 'http' }
SECURITY_GROUP_TAG_FOR_GLOBAL_HTTPS = { 'SecurityGroupType': 'cloudfront_g', 'AutoUpdate': 'true', 'Protocol': 'https' }
SECURITY_GROUP_TAG_FOR_REGION_HTTP = { 'SecurityGroupType': 'cloudfront_r', 'AutoUpdate': 'true', 'Protocol': 'http' }
SECURITY_GROUP_TAG_FOR_REGION_HTTPS = { 'SecurityGroupType': 'cloudfront_r', 'AutoUpdate': 'true', 'Protocol': 'https' }
...
def get_ranges_for_service(ranges, service, subset):
    ...
    for prefix in ranges['prefixes']:
        if prefix['service'] == service and ((subset == prefix['region'] and subset == "GLOBAL") or (subset != 'GLOBAL' and prefix['region'] != 'GLOBAL')):
            LOGGER.info(('Found ' + service + ' region: ' + prefix['region'] + ' range: ' + prefix['ip_prefix']))
...
def update_security_group(client, group, new_ranges, port):
    ...
    if len(group['IpPermissions']) > 0:
            ...
            if permission['FromPort'] <= port and permission['ToPort'] >= port :
                ...
                for range in permission['IpRanges']:
                    ...
                    if new_ranges.count(cidr) == 0:
                        ...
                        LOGGER.info((group['GroupId'] + ": Revoking " + cidr + ":" + str(permission['ToPort'])))

                for range in new_ranges:
                    if old_prefixes.count(range) == 0:
                        to_add.append({ 'CidrIp': range })
                        LOGGER.info((group['GroupId'] + ": Adding " + range + ":" + str(permission['ToPort'])))
        ...
        for range in new_ranges:
            to_add.append({ 'CidrIp': range })
            LOGGER.info((group['GroupId'] + ": Adding " + range + ":" + str(port)))
