resource "aws_iam_policy" "aws_loadbalancer_controller_policy" {
  name   = "${var.alb_controller_role_name}-policy"
  policy = <<POLICY
{
    "Version": "2012-10-17",
    "Statement": [
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "${replace(var.cluster_oidc_issuer_url, "https://", "")}:sub": "system:serviceaccount:kube-system:${var.alb_controller_service_account_name}"
        }
      }
    }
  ]
